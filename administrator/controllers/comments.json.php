...
class ImcControllerComments extends ImcController
{
	...
	public function postComment($api = false, $userid = null)
	{
		...
		try {
			...
			$comment->profile_picture_url = JURI::root().'components/com_imc/assets/images/user-icon.png';
			$comment->created_by_admin = $created_by_admin;
			$comment->created_by_current_user = true;

			if($comment->moderation)
			{
				$comment->profile_picture_url = JURI::root().'components/com_imc/assets/images/user-icon-moderated.png';
			}

			if($created_by_admin)
			{
				$comment->profile_picture_url = JURI::root().'components/com_imc/assets/images/admin-user-icon.png';
			}
